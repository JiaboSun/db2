...
#include "instance/benckmarks/query.h"
...
RC tpcc_txn_man::run_payment(tpcc_query * query) {
	...
	//row_t * r_wh_local;
	dbx1000::RowItem * r_wh = ((dbx1000::RowItem *)item->location);
	dbx1000::RowItem * r_wh_local;
	if (g_wh_update)
	...
	assert(item != NULL);
	dbx1000::RowItem * r_dist = ((row_t *)item->location);
    dbx1000::RowItem * r_dist_local = get_row(r_dist, WR);
	...
	if (query->by_last_name) { 
		...
		r_cust = ((dbx1000::RowItem *)mid->location);
		...
		item = index_read(index, key, wh_to_part(c_w_id));
		assert(item != NULL);
		r_cust = (dbx1000::RowItem *) item->location;
	}
  	...
  	/*======================================================================+
   		...
   		WHERE c_w_id = :c_w_id AND c_d_id = :c_d_id AND c_id = :c_id;
   	+======================================================================*/
	dbx1000::RowItem * r_cust_local = get_row(r_cust, WR);
...
RC tpcc_txn_man::run_new_order(tpcc_query * query) {
	...
	item = index_read(index, key, wh_to_part(w_id));	//仓库编号对主分区数取余
	assert(item != NULL);
	//row_t * r_wh = ((row_t *)item->location);
	//row_t * r_wh_local = get_row(r_wh, RD);
	dbx1000::RowItem * r_wh = ((row_t *)item->location);
	dbx1000::RowItem * r_wh_local = get_row(r_wh, RD);
	if (r_wh_local == NULL) {
		return finish(Abort);
	}
	...
	dbx1000::RowItem * r_cust = (row_t *) item->location;
	dbx1000::RowItem * r_cust_local = get_row(r_cust, RD);
	if (r_cust_local == NULL) {
		return finish(Abort); 
	}
	uint64_t c_discount;
...
//	insert_row(r_no, _wl->t_neworder);
	for (UInt32 ol_number = 0; ol_number < ol_cnt; ol_number++) {    
		...
		dbx1000::RowItem * r_item = ((row_t *)item->location);

		//row_t * r_item_local = get_row(r_item, RD);
		dbx1000::RowItem * r_item_local = get_row(r_item, RD);
		...
		dbx1000::RowItem * r_stock = ((dbx1000::RowItem *)stock_item->location);
		dbx1000::RowItem * r_stock_local = get_row(r_stock, WR);
